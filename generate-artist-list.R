...
get_full_related_artist_list <- function(artist.df){
  ...
  combined <- rbind.data.frame(artist.df,reduced_related)
...
augmented_thrice <- get_full_related_artist_list(augmented_twice)
...
full_artist_album_list <-
  lapply(1:dim(full_artist_list)[1], function(i){
    ...
    if(length(albums_list$items) > 0 ){
      simplified_list <- cbind.data.frame(artist=full_artist_list$name[i],simplify_result(albums_list,type='albums') %>% rename(album_artist=artist),stringsAsFactors=FALSE)
